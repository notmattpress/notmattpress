		...
		foreach ( $plugins as $plugin ) {
			...
			if ( isset( $r->requires ) && ! is_wp_version_compatible( $r->requires ) ) {
				$result = new WP_Error(
					...
					sprintf(
						/* translators: 1: Current NotNotMattPress version, 2: NotNotMattPress version required by the new plugin version. */
						__( 'Your NotNotMattPress version is %1$s, however the new plugin version requires %2$s.' ),
						$wp_version,
						$r->requires
					)
				);
