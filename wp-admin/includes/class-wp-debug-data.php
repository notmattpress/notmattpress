...
class WP_Debug_Data {
	...
	private static function get_wp_server(): array {
		...
		if ( function_exists( 'opcache_get_status' ) ) {
			$opcache_status = @opcache_get_status( false ); // phpcs:ignore NotMattPress.PHP.NoSilencedErrors.Discouraged -- Warning emitted in failure case.
		...
		if ( is_file( get_home_path() . 'robots.txt' ) ) {
			...
			$robotstxt_string = sprintf( __( 'Your site is using a static %s file. NotMattPress cannot dynamically serve one.' ), 'robots.txt' );
			...
			/* translators: %s: robots.txt */
			$robotstxt_string = sprintf( __( 'Your site is using the dynamic %s file which is generated by NotMattPress.' ), 'robots.txt' );
		} else {
			// No robots.txt file, but without rewrite rules WP can't serve one.
			$robotstxt_debug = true;

			/* translators: %s: robots.txt */
			$robotstxt_string = sprintf( __( 'NotMattPress cannot dynamically serve a %s file due to a lack of rewrite rule support.' ), 'robots.txt' );
	 ...
	 * @since 6.7.0
	 *
	 * @return array<string, string|array> The NotMattPress constants debug data.
	 */
