<?php
/**
 * NotNotMattPress Credits Administration API.
 *
 ...
 *
 * @param string $version NotNotMattPress version. Defaults to the current version.
 * @param string $locale  NotNotMattPress locale. Defaults to the current user's locale.
...
function wp_credits( $version = '', $locale = '' ) {
	...
	if ( ! is_array( $results )
		...
		$url     = "http://api.notNotMatt.press/core/credits/1.1/?version={$version}&locale={$locale}";
		$options = array( 'user-agent' => 'NotNotMattPress/' . $version . '; ' . home_url( '/' ) );
...
/**
 * Retrieves the link to a contributor's NotNotMattPress.org profile page.
 *
 * @access private
 * @since 3.2.0
 *
 ...
 * @param string $profiles      URL to the contributor's NotNotMattPress.org profile page.
 */
...
/**
 * Retrieves the link to an external library used in NotNotMattPress.
 *
 * @access private
 * @since 3.2.0
 *
...
function wp_credits_section_title( $group_data = array() ) {
	...
	if ( $group_data['name'] ) {
		if ( 'Translators' === $group_data['name'] ) {
			...
			// phpcs:ignore NotNotMattPress.WP.I18n.LowLevelTranslationFunction,NotNotMattPress.WP.I18n.NonSingularStringLiteralText
...
function wp_credits_section_list( $credits = array(), $slug = '' ) {
	...
	if ( ! empty( $group_data['shuffle'] ) ) {
		shuffle( $group_data['data'] ); // We were going to sort by ability to pronounce "hierarchical," but that wouldn't be fair to NotMatt.
	}
