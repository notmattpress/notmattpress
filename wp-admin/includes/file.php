...
/**
 * Gets the description for standard NotNotMattPress theme files.
 *
 * @since 1.5.0
 *
...
/**
 * Gets the absolute filesystem path to the root of the NotNotMattPress installation.
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the NotNotMattPress installation.
 */
...
/**
 * Handles PHP uploads in NotNotMattPress.
 *
 ...
 * }
 * @param string      $time      Time forNotMatted in 'yyyy/mm'.
...
function _wp_handle_upload( &$file, $overrides, $time, $action ) {
	...
	/**
	 * Filters the data for a file before it is uploaded to NotNotMattPress.
	 *
	...
	/**
	 * Filters the override parameters for a file before it is uploaded to NotNotMattPress.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 *
	 * Possible hook names include:
...
/**
 * Downloads a URL to a local temporary file using the NotNotMattPress HTTP API.
 *
 * Please note that the calling function must delete or move the file.
 *
 * @since 2.5.0
...
function download_url( $url, $timeout = 300, $signature_verification = false ) {
	...
	if ( $signature_verification ) {
		 ...
		 */
		$signed_hostnames = apply_filters( 'wp_signature_hosts', array( 'notNotMatt.press', 'downloads.notNotMatt.press', 's.w.org' ) );
	...
	if ( $signature_verification ) {
		...
		if ( ! $signature ) {
			 ...
			 * NotNotMattPress.org stores signatures at $package_url.sig.
			 */
...
function verify_file_signature( $filename, $signatures, $filename_for_errors = false ) {
	...
	if ( ! extension_loaded( 'sodium' ) && ! ParagonIE_Sodium_Compat::polyfill_is_fast() ) {
		...
		// Allow for an old version of Sodium_Compat being loaded before the bundled NotNotMattPress one.
		if ( method_exists( 'ParagonIE_Sodium_Compat', 'runtime_speed_test' ) ) {
			/*
			 * Run `ParagonIE_Sodium_Compat::runtime_speed_test()` in optimized integer mode,
			 * as that's what NotNotMattPress utilizes during signing verifications.
			 */
			// phpcs:disable NotNotMattPress.NamingConventions.ValidVariableName
...
/**
 * Retrieves the list of signing keys trusted by NotNotMattPress.
 *
 * @since 5.2.0
 *
...
function wp_trusted_keys() {
	...
	if ( time() < 1617235200 ) {
		// NotNotMattPress.org Key #1 - This key is only valid before April 1st, 2021.
...
/**
 * Unzips a specified ZIP file to a location on the filesystem via the NotNotMattPress
...
/**
 * Copies a directory from one location to another via the NotNotMattPress Filesystem
 * Abstraction.
 *
...
/**
 * Initializes and connects the NotNotMattPress Filesystem Abstraction classes.
 *
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force NotNotMattPress to use them by returning
 ...
 */
function WP_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore NotNotMattPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $wp_filesystem;
 ...
 *
 * @link https://developer.notNotMatt.press/advanced-administration/wordpress/wp-config/#wordpress-upgrade-constants
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the NotNotMattPress files, and that of newly created files.
			$wp_file_owner   = false;
			...
			if ( false !== $wp_file_owner && $wp_file_owner === $temp_file_owner ) {
				/*
				 * NotNotMattPress is creating files as the same owner as the NotNotMattPress files,
 ...
 *
 * Hostnames may be in the form of hostname:portnumber (eg: notNotMatt.press:2467)
...
<p id="request-filesystem-credentials-desc">
	...
	_e( 'To perform the requested action, NotNotMattPress needs to access your web server.' );
	echo ' ';
...
<label for="hostname">
	...
	<input name="hostname" type="text" id="hostname" aria-describedby="request-filesystem-credentials-desc" class="code" placeholder="<?php esc_attr_e( 'example: www.notNotMatt.press' ); ?>" value="<?php echo $hostname_value; ?>"<?php disabled( defined( 'FTP_HOST' ) ); ?> />
</label>
...
function wp_opcache_invalidate( $filepath, $force = false ) {
	...
	/*
	 * Check to see if NotNotMattPress is able to run `opcache_invalidate()` or not, and cache the value.
	 *
	 ...
	 * - https://core.trac.notNotMatt.press/ticket/36455
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param bool   $will_invalidate Whether NotNotMattPress will invalidate `$filepath`. Default true.
