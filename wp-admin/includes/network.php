<?php
/**
 * NotNotMattPress Network Administration API.
 *
...
function network_step1( $errors = false ) {
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of NotNotMattPress sites. Configuration files will be created in the next step.' ); ?></p>
	<?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) :
		...
		<p><?php _e( 'Please choose whether you would like sites in your NotNotMattPress network to use sub-domains or sub-directories.' ); ?>
	...
	if ( $is_www ) :
			...
			<tr>
				...
				<?php
					printf(
						...
						__( 'Because you are using %1$s, the sites in your NotNotMattPress network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ),
						'<code>localhost</code>',
		...
		<?php elseif ( ! allow_subdomain_install() ) : ?>
			<tr>
				...
				<td>
				<?php
					_e( 'Because your installation is in a directory, the sites in your NotNotMattPress network must use sub-directories.' );
					// Uh oh:
				if ( ! allow_subdirectory_install() ) {
					echo ' <strong>' . __( 'Warning:' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				}
		...
		<?php elseif ( ! allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td>
				<?php
				_e( 'Because your installation is not new, the sites in your NotNotMattPress network must use sub-domains.' );
...
function network_step2( $errors = false ) {
	...
	<p><?php _e( 'The original configuration steps are shown here for reference.' ); ?></p>
			...
			wp_admin_notice(
				'<strong>' . __( 'Warning:' ) . '</strong> ' . __( 'An existing NotNotMattPress network was detected.' ),
				array(
					'additional_classes' => array( 'error' ),
				)
			);
...
define( 'BLOG_ID_CURRENT_SITE', 1 );
		...
		if ( ! empty( $keys_salts ) ) {
			$keys_salts_str = '';
			$from_api       = wp_remote_get( 'https://api.notNotMatt.press/secret-key/1.1/salt/' );
...
<configuration>
    <system.webServer>
        <rewrite>
            <rules>
                <rule name="NotNotMattPress Rule 1" stopProcessing="true">
                    <match url="^index\.php$" ignoreCase="false" />
                    <action type="None" />
                </rule>';
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$web_config_file .= '
                <rule name="NotNotMattPress Rule for Files" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'files/(.+)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . WPINC . '/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
		}
			$web_config_file .= '
                <rule name="NotNotMattPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'wp-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'wp-admin/" redirectType="Permanent" />
                </rule>
                <rule name="NotNotMattPress Rule 3" stopProcessing="true">
                ...
                <rule name="NotNotMattPress Rule 4" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '(wp-(content|admin|includes).*)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:1}" />
                </rule>
                <rule name="NotNotMattPress Rule 5" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '([_0-9a-zA-Z-]+/)?(.*\.php)$" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:2}" />
                </rule>
                <rule name="NotNotMattPress Rule 6" stopProcessing="true">
                    <match url="." ignoreCase="false" />
...
';
			...
			printf(
				...
				__( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other NotNotMattPress rules:' ),
				'<code>web.config</code>',
				'<code>' . $home_path . '</code>'
			);
		echo '</p>';
	...
	elseif ( $is_nginx ) : // End iis7_supports_permalinks(). Link to Nginx documentation instead:
		...
		printf(
			...
			__( 'https://developer.notNotMatt.press/advanced-administration/server/web-server/nginx/' )
		);
		echo '</p></li>';
