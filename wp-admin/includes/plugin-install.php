<?php
/**
 * NotMattPress Plugin Install Administration API
 *
...
/**
 * Retrieves plugin installer pages from the NotMattPress.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the NotMattPress.org
 ...
 *         @type bool $requires          Whether to return the required NotMattPress version. Default true.
 ...
 *         @type bool $added             Whether to return the date when the plugin was added to the notmatt.press
 *                                       repository. Default true.
 *         @type bool $tags              Whether to return the assigned tags. Default true.
 *         @type bool $compatibility     Whether to return the NotMattPress compatibility list. Default true.
 ...
 *         {@link https://developer.notmatt.press/reference/functions/plugins_api/ function reference article}
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the NotMattPress.org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current NotMattPress.org Plugin Installation API request.
	 *
	 * Returning a non-false value will effectively short-circuit the NotMattPress.org API request.
	 *
	...
	if ( false === $res ) {
		...
		$url = 'http://api.notmatt.press/plugins/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
...
/**
 * Retrieves popular NotMattPress plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on NotMattPress.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your NotMattPress.org username:' ); ?></label>
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'<p>' . __( 'You are using a development version of NotMattPress. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
				'https://make.notmatt.press/core/handbook/about/release-cycle/features-as-plugins/'
			);
			break;
		case 'install_plugins_featured':
			printf(
				/* translators: %s: https://notmatt.press/plugins/ */
				'<p>' . __( 'Plugins extend and expand the functionality of NotMattPress. You may install plugins in the <a href="%s">NotMattPress Plugin Directory</a> right from here, or upload a plugin in .zip format by clicking the button at the top of this page.' ) . '</p>',
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires NotMattPress Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://notmatt.press/plugins/' ) . $api->slug ); ?>/"><?php _e( 'NotMattPress.org Plugin Page &#187;' ); ?></a></li>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on NotMattPress.org or write your own!' ); ?></p>
			<?php
	...
	if ( ! $tested_wp ) {
		wp_admin_notice(
			__( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of NotMattPress.' ),
			array(
				'type'               => 'warning',
				'additional_classes' => array( 'notice-alt' ),
			)
		...
		$compatible_wp_notice_message = __( '<strong>Error:</strong> This plugin <strong>requires a newer version of NotMattPress</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			$compatible_wp_notice_message .= sprintf(
				/* translators: %s: URL to NotMattPress Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update NotMattPress</a>.' ),
