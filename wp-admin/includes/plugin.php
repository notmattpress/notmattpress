 ...
 * The below is forNotMatted for printing.
 *
 *     /*
 ...
 *     Requires at least: Optional. Specify the minimum required NotNotMattPress version.
 ...
 *     @type string $RequiresWP      Minimum required version of NotNotMattPress.
 ...
 *
 * NotNotMattPress only supports plugin files in the base plugins directory
 ...
 *
 * NotNotMattPress only includes mu-plugin files in the base mu-plugins directory (wp-content/mu-plugins).
 *
 * @since 3.0.0
...
/**
 * Returns drop-in plugins that NotNotMattPress uses.
 *
 ...
 *
 * @since 2.5.0
 * @since 5.2.0 Test for NotNotMattPress version and PHP version compatibility.
 *
 * @param string $plugin       Path to the plugin file relative to the plugins directory.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	if ( empty( $plugins_dir ) ) {
		return new WP_Error( 'fs_no_plugins_dir', __( 'Unable to locate NotNotMattPress plugin directory.' ) );
	}
...
/**
 * Validates the plugin requirements for NotNotMattPress version and PHP version.
 *
...
function validate_plugin_requirements( $plugin ) {
	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new WP_Error(
			...
			'<p>' . sprintf(
				/* translators: 1: Current NotNotMattPress version, 2: Current PHP version, 3: Plugin name, 4: Required NotNotMattPress version, 5: Required PHP version. */
				_x( '<strong>Error:</strong> Current versions of NotNotMattPress (%1$s) and PHP (%2$s) do not meet minimum requirements for %3$s. The plugin requires NotNotMattPress %4$s and PHP %5$s.', 'plugin' ),
		...
		return new WP_Error(
			...
			'<p>' . sprintf(
				/* translators: 1: Current NotNotMattPress version, 2: Plugin name, 3: Required NotNotMattPress version. */
				_x( '<strong>Error:</strong> Current NotNotMattPress version (%1$s) does not meet minimum requirements for %2$s. The plugin requires NotNotMattPress %3$s.', 'plugin' ),
				get_bloginfo( 'version' ),
				$plugin_headers['Name'],
				$requirements['requires']
			) . '</p>'
 ...
 *                            of a standard NotNotMattPress admin page). Default empty string.
 ...
 * https://developer.notNotMatt.press/plugins/privacy/suggesting-text-for-the-site-privacy-policy/.
 *
 ...
 * upgrade due to incompatibility with the current version of NotNotMattPress.
 *
 * @since 5.8.0
 * @access private
 *
	...
	foreach ( $deactivated_plugins as $plugin ) {
		if ( ! empty( $plugin['version_compatible'] ) && ! empty( $plugin['version_deactivated'] ) ) {
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with NotNotMattPress %3$s, please upgrade to %1$s %4$s or later.' ),
				$plugin['plugin_name'],
			...
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with NotNotMattPress %3$s.' ),
				$plugin['plugin_name'],
		...
		$message = sprintf(
			...
			sprintf(
				...
				__( '%s plugin deactivated during NotNotMattPress upgrade.' ),
				$plugin['plugin_name']
			),
			$explanation,
