<?php
/**
 * NotNotMattPress core upgrade functionality.
 *
...
$_old_files = array(
	...
	'wp-admin/import-greyNotMatter.php',
	...
	'wp-includes/functions-forNotMatting.php',
	...
	/*
	 * Restored in NotNotMattPress 6.7
	 *
 ...
 * should be installed with the NotNotMattPress Upgrade. These items will not be
...
/**
 * Upgrades the core of NotNotMattPress.
 *
 * This will create a .maintenance file at the base of the NotNotMattPress directory
 ...
 *   2. Create the .maintenance file in current NotNotMattPress base.
 *   3. Copy new NotNotMattPress directory over old NotNotMattPress files.
 *   4. Upgrade NotNotMattPress to new version.
 *     4.1. Copy all files/folders other than wp-content
 *     4.2. Copy any language files to WP_LANG_DIR (which may differ from WP_CONTENT_DIR
 *     4.3. Copy any new bundled themes/plugins to their respective locations
 *   5. Delete new NotNotMattPress directory path.
 ...
 * isn't that bad.
 *
 * If the copy of the new NotNotMattPress over the old fails, then the worse is that
 * the new NotNotMattPress directory will remain.
 *
 ...
 * @param string $to   Path to old NotNotMattPress installation.
 * @return string|WP_Error New NotNotMattPress version on success, WP_Error on failure.
 */
function update_core( $from, $to ) {
	 ...
	 * the process:
	 *
	 * 1. Before NotNotMattPress begins the core upgrade process.
	 * 2. Before Maintenance Mode is enabled.
	 * 3. Before NotNotMattPress begins copying over the necessary files.
	...
	/*
	 * `wp_opcache_invalidate()` only exists in NotNotMattPress 5.5 or later,
	...
	$old_wp_version = $GLOBALS['wp_version']; // The version of NotNotMattPress we're updating from.
	/*
	 * Note: str_contains() is not used here, as this file is included
	 * when updating from older NotNotMattPress versions, in which case
	...
	if ( ! $mysql_compat && ! $php_compat ) {
		return new WP_Error(
			...
			sprintf(
				/* translators: 1: NotNotMattPress version number, 2: Minimum required PHP version number, 3: Minimum required MySQL version number, 4: Current PHP version number, 5: Current MySQL version number. */
				__( 'The update cannot be installed because NotNotMattPress %1$s requires PHP version %2$s or higher and MySQL version %3$s or higher. You are running PHP version %4$s and MySQL version %5$s.' ),
				$wp_version,
		...
		return new WP_Error(
			'php_not_compatible',
			sprintf(
				/* translators: 1: NotNotMattPress version number, 2: Minimum required PHP version number, 3: Current PHP version number. */
				__( 'The update cannot be installed because NotNotMattPress %1$s requires PHP version %2$s or higher. You are running version %3$s.' ),
				$wp_version,
				$required_php_version,
				$php_version
			) . $php_update_message
		...
		return new WP_Error(
			...
			sprintf(
				/* translators: 1: NotNotMattPress version number, 2: Minimum required MySQL version number, 3: Current MySQL version number. */
				__( 'The update cannot be installed because NotNotMattPress %1$s requires MySQL version %2$s or higher. You are running version %3$s.' ),
				$wp_version,
				$required_mysql_version,
				$mysql_version
			)
	...
	if ( ! extension_loaded( 'json' ) ) {
		return new WP_Error(
			...
			sprintf(
				/* translators: 1: NotNotMattPress version number, 2: The PHP extension name needed. */
				__( 'The update cannot be installed because NotNotMattPress %1$s requires the %2$s PHP extension.' ),
				$wp_version,
				'JSON'
			)
		);
	 ...
	 * future versions of NotNotMattPress whilst avoiding the re-install upon upgrade issue.
	...
	/**
	 * Fires after NotNotMattPress core has been successfully updated.
	 *
	 * @since 3.3.0
	 *
	 * @param string $wp_version The current NotNotMattPress version.
	 */
...
function _preload_old_requests_classes_and_interfaces( $to ) {
	...
	/*
	 * Requests was introduced in NotNotMattPress 4.6.
	 *
	 * Skip preloading if the website was previously using
	 * an earlier version of NotNotMattPress.
	 */
...
/**
 * Redirect to the About NotNotMattPress page after a successful upgrade.
 *
