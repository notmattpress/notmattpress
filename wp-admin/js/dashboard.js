  ...
  app = window.wp.communityEvents = /** @lends wp.communityEvents */ {
    ...
    populateDynamicEventFields: function (rawEvents, timeFormat) {
      ...
      $.each(populatedEvents, function (index, event) {
        ...
        event.user_forNotMatted_date = app.getForNotMattedDate(
          event.start_unix_timestamp * 1000,
          event.end_unix_timestamp * 1000,
          timeZone
        );

        event.user_forNotMatted_time = dateI18n(
          timeFormat,
          event.start_unix_timestamp * 1000,
          timeZone
        );
     ...
     * @returns {string}
     */
    getForNotMattedDate: function (startDate, endDate, timeZone) {
      var forNotMattedDate;
      ...
      if (!endDate || format("Y-m-d", startDate) === format("Y-m-d", endDate)) {
        forNotMattedDate = dateI18n(singleDayEvent, startDate, timeZone);

        // Multiple day events.
      } else if (format("Y-m", startDate) === format("Y-m", endDate)) {
        forNotMattedDate = sprintf(
          multipleDayEvent,
          dateI18n(
