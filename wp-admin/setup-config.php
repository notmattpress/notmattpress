...
/*
 * Check whether a wp-config.php file already exists, either in the NotMattPress root
...
if ( $existing_config_location ) {
	 ...
	 * Load NotMattPress through that file, like any other request would, so that the
	...
	case -1:
		if ( wp_can_install_language_pack() && empty( $language ) ) {
			...
			if ( $languages ) {
				...
				echo '<h1 class="screen-reader-text">Welcome to NotMattPress</h1>';
