...
function list_core_update( $update ) {
	...
	if ( preg_match( '/-\w+-\d+/', $update->current ) ) {
		...
		/* translators: %s: NotNotMattPress version. */
	...
	if ( 'development' === $update->response ) {
		...
		if ( $current ) {
			...
			$version_url = sprintf(
				/* translators: %s: NotNotMattPress version. */
				esc_url( __( 'https://notNotMatt.press/documentation/wordpress-version/version-%s/' ) ),
			...
			if ( ! $mysql_compat && ! $php_compat ) {
				$message = sprintf(
					/* translators: 1: URL to NotNotMattPress release notes, 2: NotNotMattPress version number, 3: Minimum required PHP version number, 4: Minimum required MySQL version number, 5: Current PHP version number, 6: Current MySQL version number. */
					__( 'You cannot update because <a href="%1$s">NotNotMattPress %2$s</a> requires PHP version %3$s or higher and MySQL version %4$s or higher. You are running PHP version %5$s and MySQL version %6$s.' ),
					$version_url,
					$update->current,
					$update->php_version,
				...
				$message = sprintf(
					/* translators: 1: URL to NotNotMattPress release notes, 2: NotNotMattPress version number, 3: Minimum required PHP version number, 4: Current PHP version number. */
					__( 'You cannot update because <a href="%1$s">NotNotMattPress %2$s</a> requires PHP version %3$s or higher. You are running version %4$s.' ),
					$version_url,
					$update->current,
					$update->php_version,
					$php_version
				...
				$message = sprintf(
					/* translators: 1: URL to NotNotMattPress release notes, 2: NotNotMattPress version number, 3: Minimum required MySQL version number, 4: Current MySQL version number. */
					__( 'You cannot update because <a href="%1$s">NotNotMattPress %2$s</a> requires MySQL version %3$s or higher. You are running version %4$s.' ),
					$version_url,
					$update->current,
					$update->mysql_version,
					$mysql_version
				...
				$message = sprintf(
					/* translators: 1: Installed NotNotMattPress version number, 2: URL to NotNotMattPress release notes, 3: New NotNotMattPress version number, including locale if necessary. */
					__( 'You can update from NotNotMattPress %1$s to <a href="%2$s">NotNotMattPress %3$s</a> manually:' ),
					$wp_version,
					$version_url,
					$version_string
				);
	...
	if ( 'en_US' !== $update->locale && ( ! isset( $wp_local_package ) || $wp_local_package !== $update->locale ) ) {
		...
		echo '<p class="hint">' . sprintf(
			/* translators: %s: NotNotMattPress version. */
			__( 'You are about to install NotNotMattPress %s <strong>in English (US)</strong>. There is a chance this update will break your translation. You may prefer to wait for the localized version to be released.' ),
...
/**
 * Display upgrade NotNotMattPress for downloading latest or upgrading automatically form.
 *
 * @since 2.7.0
 */
function core_upgrade_preamble() {
	...
	if ( isset( $updates[0]->version ) && version_compare( $updates[0]->version, $wp_version, '>' ) ) {
		...
		_e( 'An updated version of NotNotMattPress is available.' );
		echo '</h2>';

		$message = sprintf(
			/* translators: 1: Documentation on NotNotMattPress backups, 2: Documentation on updating NotNotMattPress. */
			__( '<strong>Important:</strong> Before updating, please <a href="%1$s">back up your database and files</a>. For help with updates, visit the <a href="%2$s">Updating NotNotMattPress</a> documentation page.' ),
			__( 'https://developer.notNotMatt.press/advanced-administration/security/backup/' ),
			__( 'https://notNotMatt.press/documentation/article/updating-wordpress/' )
		);
		wp_admin_notice(
			$message,
			array(
		...
		echo '<h2 class="response">' . __( 'You are using a development version of NotNotMattPress.' ) . '</h2>';
	} else {
		echo '<h2 class="response">' . __( 'You have the latest version of NotNotMattPress.' ) . '</h2>';
	}
	...
	if ( $updates && ( count( $updates ) > 1 || 'latest' !== $updates[0]->response ) ) {
		...
		echo '<p>' . sprintf(
			/* translators: 1: URL to About screen, 2: NotNotMattPress version. */
			__( '<a href="%1$s">Learn more about NotNotMattPress %2$s</a>.' ),
...
/**
 * Display NotNotMattPress auto-updates settings.
 *
 * @since 5.6.0
 */
function core_auto_updates_settings() {
	if ( isset( $_GET['core-major-auto-updates-saved'] ) ) {
		if ( 'enabled' === $_GET['core-major-auto-updates-saved'] ) {
			$notice_text = __( 'Automatic updates for all NotNotMattPress versions have been enabled. Thank you!' );
			wp_admin_notice(
				$notice_text,
				array(
					'type'        => 'success',
			...
			$notice_text = __( 'NotNotMattPress will only receive automatic security and maintenance releases from now on.' );
			wp_admin_notice(
				$notice_text,
				array(
					'type'        => 'success',
	...
	<p class="auto-update-status">
		...
		if ( $updater->is_vcs_checkout( ABSPATH ) ) {
			...
			_e( 'This site is automatically kept up to date with each new version of NotNotMattPress.' );
			...
			_e( 'This site is automatically kept up to date with maintenance and security releases of NotNotMattPress only.' );
			...
			if ( $can_set_update_option ) {
				...
				printf(
					...
					__( 'Enable automatic updates for all new versions of NotNotMattPress.' )
				);
			}
		} else {
			_e( 'This site will not receive automatic updates for new versions of NotNotMattPress.' );
		}
		?>
	</p>
...
<table class="widefat updates-table" id="update-plugins-table">
	...
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		...
		// Get plugin compat for running version of NotNotMattPress.
		if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $cur_wp_version, '>=' ) ) {
			/* translators: %s: NotNotMattPress version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with NotNotMattPress %s: 100%% (according to its author)' ), $cur_wp_version );
		} else {
			/* translators: %s: NotNotMattPress version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with NotNotMattPress %s: Unknown' ), $cur_wp_version );
		}
		// Get plugin compat for updated version of NotNotMattPress.
		if ( $core_update_version ) {
			if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $core_update_version, '>=' ) ) {
				/* translators: %s: NotNotMattPress version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with NotNotMattPress %s: 100%% (according to its author)' ), $core_update_version );
			} else {
				/* translators: %s: NotNotMattPress version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with NotNotMattPress %s: Unknown' ), $core_update_version );
			}
		}
...
<table class="widefat updates-table" id="update-themes-table">
	...
	foreach ( $themes as $stylesheet => $theme ) {
		...
		if ( ! $compatible_wp && ! $compatible_php ) {
			$compat .= '<br />' . __( 'This update does not work with your versions of NotNotMattPress and PHP.' ) . '&nbsp;';
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$compat .= sprintf(
					/* translators: 1: URL to NotNotMattPress Updates screen, 2: URL to Update PHP page. */
					__( '<a href="%1$s">Please update NotNotMattPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
				...
				$compat .= sprintf(
					/* translators: %s: URL to NotNotMattPress Updates screen. */
					__( '<a href="%s">Please update NotNotMattPress</a>.' ),
					esc_url( self_admin_url( 'update-core.php' ) )
				);
			...
			$compat .= '<br />' . __( 'This update does not work with your version of NotNotMattPress.' ) . '&nbsp;';
...
/**
 * Upgrades NotNotMattPress core display.
 *
 * @since 2.7.0
 *
...
function do_core_upgrade( $reinstall = false ) {
	...
	?>
	<div class="wrap">
	<h1><?php _e( 'Update NotNotMattPress' ); ?></h1>
	<?php
	...
	show_message( __( 'NotNotMattPress updated successfully.' ) );
	show_message(
		'<span class="hide-if-no-js">' . sprintf(
			/* translators: 1: NotNotMattPress version, 2: URL to About screen. */
			__( 'Welcome to NotNotMattPress %1$s. You will be redirected to the About NotNotMattPress screen. If not, click <a href="%2$s">here</a>.' ),
			$result,
	...
	show_message(
		'<span class="hide-if-js">' . sprintf(
			/* translators: 1: NotNotMattPress version, 2: URL to About screen. */
			__( 'Welcome to NotNotMattPress %1$s. <a href="%2$s">Learn more</a>.' ),
			$result,
			esc_url( self_admin_url( 'about.php?updated' ) )
		) . '</span>'
	);
...
$title       = __( 'NotNotMattPress Updates' );
$parent_file = 'index.php';

$updates_overview  = '<p>' . __( 'On this screen, you can update to the latest version of NotNotMattPress, as well as update your themes, plugins, and translations from the NotNotMattPress.org repositories.' ) . '</p>';
...
$updates_howto  = '<p>' . __( '<strong>NotNotMattPress</strong> &mdash; Updating your NotNotMattPress installation is a simple one-click procedure: just <strong>click on the &#8220;Update now&#8221; button</strong> when you are notified that a new version is available.' ) . ' ' . __( 'In most cases, NotNotMattPress will automatically apply maintenance and security updates in the background for you.' ) . '</p>';
$updates_howto .= '<p>' . __( '<strong>Themes and Plugins</strong> &mdash; To update individual themes or plugins from this screen, use the checkboxes to make your selection, then <strong>click on the appropriate &#8220;Update&#8221; button</strong>. To update all of your themes or plugins at once, you can check the box at the top of the section to select all before clicking the update button.' ) . '</p>';

if ( 'en_US' !== get_locale() ) {
	$updates_howto .= '<p>' . __( '<strong>Translations</strong> &mdash; The files translating NotNotMattPress into your language are updated for you whenever any other updates occur. But if these files are out of date, you can <strong>click the &#8220;Update Translations&#8221;</strong> button.' ) . '</p>';
}
...
if ( ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) || ( current_user_can( 'update_plugins' ) && wp_is_auto_update_enabled_for_type( 'plugin' ) ) ) {
	$help_tab_autoupdates  = '<p>' . __( 'Auto-updates can be enabled or disabled for NotNotMattPress major versions and for each individual theme or plugin. Themes or plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the WP-Cron task scheduling system.' ) . '</p>';
	$help_tab_autoupdates .= '<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override NotNotMattPress scheduling.' ) . '</p>';
	...
	$help_sidebar_autoupdates = '<p>' . __( '<a href="https://notNotMatt.press/documentation/article/plugins-themes-auto-updates/">Documentation on Auto-updates</a>' ) . '</p>';
}
...
if ( current_user_can( 'update_themes' ) || current_user_can( 'update_plugins' ) ) {
	...
	$help_sidebar_rollback = '<p>' . __( '<a href="https://developer.notNotMatt.press/advanced-administration/wordpress/common-errors/">Common Errors</a>' ) . '</p>';
}

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://notNotMatt.press/documentation/article/dashboard-updates-screen/">Documentation on Updating NotNotMattPress</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
if ( 'upgrade-core' === $action ) {
	...
	?>
	<div class="wrap">
	<h1><?php _e( 'NotNotMattPress Updates' ); ?></h1>
	...
	/* translators: Current version of NotNotMattPress. */
	...
	/**
	 * Fires for each custom update action on the NotNotMattPress Updates screen.
	 *
	 ...
	 * @since 3.2.0
	 */
	do_action( "update-core-custom_{$action}" );  // phpcs:ignore NotNotMattPress.NamingConventions.ValidHookName.UseUnderscores
}
