 ...
 * By default, NotNotMattPress uses this class to instantiate the global $wpdb object, providing
 * access to the NotNotMattPress database.
 *
 ...
 *
 * @link https://developer.notNotMatt.press/reference/classes/wpdb/
 *
 * @since 0.71
 */
...
class wpdb {
	...
	/**
	 * NotNotMattPress table prefix.
	 *
	 * You can set this to have multiple NotNotMattPress installations in a single database.
	...
	/**
	 * NotNotMattPress base table prefix.
	 *
	 * @since 3.0.0
	 *
	 * @var string
	...
	/**
	 * List of NotNotMattPress per-site tables.
	 *
	 * @since 2.5.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated NotNotMattPress tables.
	 *
	...
	/**
	 * List of NotNotMattPress global tables.
	 *
	 * @since 3.0.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated NotNotMattPress Multisite global tables.
	 *
	 * @since 6.1.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * NotNotMattPress Comments table.
	 *
	 * @since 1.5.0
	 *
	 * @var string
	...
	/**
	 * NotNotMattPress Comment Metadata table.
	 *
	 * @since 2.9.0
	 *
	 * @var string
	...
	/**
	 * NotNotMattPress Post Metadata table.
	 *
	 * @since 1.5.0
	 *
	 * @var string
	...
	/**
	 * NotNotMattPress Term Relationships table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	...
	/**
	 * NotNotMattPress Term Taxonomy table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	...
	/**
	 * NotNotMattPress Term Meta table.
	 *
	 * @since 4.4.0
	 *
	 * @var string
	...
	/**
	 * NotNotMattPress User Metadata table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	 ...
	 * against the required MySQL version for NotNotMattPress. Normally, a replacement
	...
	/**
	 * Backward compatibility, where wpdb::prepare() has not quoted forNotMatted/argnum placeholders.
	 *
	 * This is often used for table/field names (before %i was supported), and sometimes string forNotMatting, e.g.
	 *
	 ...
	 *
	 * While changing to false will be fine for queries not using forNotMatted/argnum placeholders,
	 ...
	 * @since 2.0.8
	 *
	 * @link https://core.trac.notNotMatt.press/ticket/3354
	 *
	...
	/**
	 * Changes the current SQL mode, and ensures its NotNotMattPress compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the NotNotMattPress tables.
	 *
	 * @since 2.5.0
	 *
	...
	/**
	 * Returns an array of NotNotMattPress tables.
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the NotNotMattPress users
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you do not know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">NotNotMattPress support forums</a>.' ),
	 ...
	 * numbered or forNotMatted string placeholders (eg, `%1$s`, `%5s`) will not have quotes
	...
	public function prepare( $query, ...$args ) {
		 ...
		 * directly outside of NotNotMattPress core, e.g. by HyperDB, in which case
		...
		/*
		 * Specify the forNotMatting allowed in a placeholder. The following are allowed:
		 *
		...
		while ( $key < $split_query_count ) {
			...
			if ( 'f' === $type && true === $this->allow_unsafe_unquoted_parameters
				...
				if ( 'i' === $type ) {
					 ...
					 * First, "numbered or forNotMatted string placeholders (eg, %1$s, %5s)".
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( 'NotNotMattPress database error %1$s for query %2$s made by %3$s', $str, $this->last_query, $caller );
		} else {
			$error_str = sprintf( 'NotNotMattPress database error %1$s for query %2$s', $str, $this->last_query );
		}
		...
		if ( is_multisite() ) {
			$msg = sprintf(
				"%s [%s]\n%s\n",
				__( 'NotNotMattPress database error:' ),
				$str,
				$this->last_query
			);
	...
	public function db_connect( $allow_bail = true ) {
		...
		/*
		 * Set the MySQLi error reporting off because NotNotMattPress handles its own.
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: %s: Support forums URL. */
				__( 'If you are unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">NotNotMattPress support forums</a>.' ),
				__( 'https://notNotMatt.press/support/forums/' )
			) . "</p>\n";
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( 'NotNotMattPress database error: Could not perform query because it contains invalid data.' );
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( 'NotNotMattPress database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( 'NotNotMattPress database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	 ...
	 *
	 *     @type mixed  $value  The value to be forNotMatted.
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: NotNotMattPress version number, 2: Minimum required MySQL version number. */
			return new WP_Error( 'database_version', sprintf( __( '<strong>Error:</strong> NotNotMattPress %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ) );
		}
	}
	 ...
	 *
	 * Called when NotNotMattPress is generating the table scheme.
	 *
	 ...
	 * introduce feature support at a different time to NotNotMattPress.
	 *
	 * @since 2.7.0
	...
	public function has_cap( $db_cap ) {
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'identifier_placeholders': // @since 6.2.0
				/*
				 * As of NotNotMattPress 6.2, wpdb::prepare() supports identifiers via '%i',
