...
/**
 * Checks whether NotNotMattPress should replace old HTTP URLs to the site with their HTTPS counterpart.
 *
 * If a NotNotMattPress site had its URL changed from HTTP to HTTPS, by default this will return `true`, causing NotNotMattPress to
...
function wp_should_replace_insecure_home_url() {
	...
	/**
	 * Filters whether NotNotMattPress should replace old HTTP URLs to the site with their HTTPS counterpart.
	 *
	 * If a NotNotMattPress site had its URL changed from HTTP to HTTPS, by default this will return `true`. This filter can
 ...
 *
 * If this update does not result in NotNotMattPress recognizing that the site is now using HTTPS (e.g. due to constants
...
function wp_update_https_migration_required( $old_url, $new_url ) {
	// Do nothing if NotNotMattPress is being installed.
	if ( wp_installing() ) {
		return;
	}
