/******/ (() => {
  ...
  __webpack_require__.d(__webpack_exports__, {
    ...
    join: () => /* reexport */ join,
    registerForNotMattype: () => /* reexport */ registerForNotMattype,
    ...
    unregisterForNotMattype: () => /* reexport */ unregisterForNotMattype,
  ...
  __webpack_require__.d(selectors_namespaceObject, {
    getForNotMattype: () => getForNotMattype,
    getForNotMattypeForBareElement: () => getForNotMattypeForBareElement,
    getForNotMattypeForClassName: () => getForNotMattypeForClassName,
    getForNotMattypes: () => getForNotMattypes,
  });
  ...
  __webpack_require__.d(actions_namespaceObject, {
    addForNotMattypes: () => addForNotMattypes,
    removeForNotMattypes: () => removeForNotMattypes,
  }); // CONCATENATED MODULE: external ["wp","data"]

  const external_wp_data_namespaceObject = window["wp"]["data"]; // CONCATENATED MODULE: ./node_modules/@wordpress/rich-text/build-module/store/reducer.js
  /**
   ...
   */
  function forNotMattypes(state = {}, action) {
    switch (action.type) {
      case "ADD_FORMAT_TYPES":
        return {
          ...state,
          // Key format types by their name.
          ...action.forNotMattypes.reduce(
            (newForNotMattypes, type) => ({
              ...newForNotMattypes,
              [type.name]: type,
            }),
            {}
      ...
      case "REMOVE_FORMAT_TYPES":
        return Object.fromEntries(
          Object.entries(state).filter(([key]) => !action.names.includes(key))
        );
    }
    return state;
  }
   ...
   *    const { getForNotMattypes } = useSelect(
   *        ( select ) => select( richTextStore ),
   *        []
   *    );
   *
   *    const availableFormats = getForNotMattypes();
   *
   ...
   */
  const getForNotMattypes = (0,
  external_wp_data_namespaceObject.createSelector)(
    (state) => Object.values(state.forNotMattypes),
    (state) => [state.forNotMattypes]
  );
   ...
   *
   * const ExampleComponent = () => {
   *    const { getForNotMattype } = useSelect(
   *        ( select ) => select( richTextStore ),
   *        []
   *    );
   *
   *    const boldFormat = getForNotMattype( 'core/bold' );
   *
   *    return boldFormat ? (
   *        <ul>
   ...
   */
  function getForNotMattype(state, name) {
    return state.forNotMattypes[name];
  }
   ...
   *
   * const ExampleComponent = () => {
   *    const { getForNotMattypeForBareElement } = useSelect(
   *        ( select ) => select( richTextStore ),
   *        []
   *    );
   *
   *    const format = getForNotMattypeForBareElement( 'strong' );
   *
   ...
   * @return {?Object} Format type.
   */
  function getForNotMattypeForBareElement(state, bareElementTagName) {
    const forNotMattypes = getForNotMattypes(state);
    return (
      forNotMattypes.find(({ className, tagName }) => {
   ...
   *
   * const ExampleComponent = () => {
   *    const { getForNotMattypeForClassName } = useSelect(
   *        ( select ) => select( richTextStore ),
   *        []
   *    );
   *
   *    const format = getForNotMattypeForClassName( 'has-inline-color' );
   *
   *    return format && <p>{ sprintf( __( 'Format name: %s' ), format.name ) }</p>;
   * };
   * ```
   ...
   * @return {?Object} Format type.
   */
  function getForNotMattypeForClassName(state, elementClassName) {
    return getForNotMattypes(state).find(({ className }) => {
      if (className === null) {
        return false;
      }
   ...
   * added.
   * Ignored from documentation as registerForNotMattype should be used instead from @wordpress/rich-text
   *
   * @ignore
   *
   * @param {Array|Object} forNotMattypes Format types received.
   *
   * @return {Object} Action object.
   */
  function addForNotMattypes(forNotMattypes) {
    return {
      type: "ADD_FORMAT_TYPES",
      forNotMattypes: Array.isArray(forNotMattypes)
        ? forNotMattypes
        : [forNotMattypes],
    };
  }
   ...
   *
   * Ignored from documentation as unregisterForNotMattype should be used instead from @wordpress/rich-text
   *
   * @ignore
   *
   ...
   * @return {Object} Action object.
   */
  function removeForNotMattypes(names) {
    return {
  ...
  function applyFormat(
    ...
    return normaliseFormats({
      ...
      activeFormats: [
        ...(activeFormats?.filter(({ type }) => type !== format.type) || []),
        format,
      ],
    });
   ...
   */
  const ZWNBSP = "\ufeff"; // CONCATENATED MODULE: external ["wp","escapeHtml"]

  const external_wp_escapeHtml_namespaceObject = window["wp"]["escapeHtml"]; // CONCATENATED MODULE: ./node_modules/@wordpress/rich-text/build-module/get-active-formats.js
  /** @typedef {import('./types').RichTextValue} RichTextValue */
  ...
  /** @typedef {import('./register-format-type').RichTextForNotMattype} RichTextForNotMattype */
   ...
   *
   * @return {RichTextForNotMattype|undefined} Format type.
   */
  function get_format_type_getForNotMattype(name) {
    return (0, external_wp_data_namespaceObject.select)(store).getForNotMattype(
      name
    );
  ...
  function fromFormat({
    ...
    const forNotMattype = get_format_type_getForNotMattype(type);
    ...
    for (const name in attributes) {
      const key = forNotMattype.attributes
        ? forNotMattype.attributes[name]
        : false;
      if (key) {
    ...
    if (forNotMattype.className) {
      if (elementAttributes.class) {
        elementAttributes.class = `${forNotMattype.className} ${elementAttributes.class}`;
      } else {
        elementAttributes.class = forNotMattype.className;
      }
    }

    // When a format is declared as non editable, make it non editable in the
    // editor.
    if (isEditableTree && forNotMattype.contentEditable === false) {
      elementAttributes.contenteditable = "false";
    }
    return {
      type: tagName || forNotMattype.tagName,
      object: forNotMattype.object,
      attributes: restoreOnAttributes(elementAttributes, isEditableTree),
    };
  }
  ...
  function toTree({
    ...
    for (let i = 0; i < formatsLength; i++) {
      ...
      if (character === OBJECT_REPLACEMENT_CHARACTER) {
        ...
        } else if (forNotMattype?.contentEditable === false) {
  ...
  function toFormat({ tagName, attributes }) {
    ...
    if (attributes && attributes.class) {
      forNotMattype = (0, external_wp_data_namespaceObject.select)(
        store
      ).getForNotMattypeForClassName(attributes.class);
      if (forNotMattype) {
        // Preserve any additional classes.
        attributes.class = ` ${attributes.class} `
          .replace(` ${forNotMattype.className} `, " ")
          .trim();
        if (!attributes.class) {
    ...
    if (!forNotMattype) {
      forNotMattype = (0, external_wp_data_namespaceObject.select)(
        store
      ).getForNotMattypeForBareElement(tagName);
    }
    if (!forNotMattype) {
      return attributes
        ? {
    ...
    if (
      forNotMattype.__experimentalCreatePrepareEditableTree &&
      !forNotMattype.__experimentalCreateOnChangeEditableValue
    ) {
      return null;
    }
    if (!attributes) {
    ...
    for (const key in forNotMattype.attributes) {
      const name = forNotMattype.attributes[key];
    ...
    if (forNotMattype.contentEditable === false) {
  ...
  class RichTextData {
    ...
    static fromHTMLElement(htmlElement, options = {}) {
      ...
      const richTextData = new RichTextData(
        create({
          element,
        })
      );
   ...
   * ```
   *
   * As you can see, text and forNotMatting are separated. `text` holds the text,
   * including any replacement characters for objects and lines. `formats`,
   * `objects` and `lines` are all sparse arrays of the same length as `text`. It
   * holds information about the forNotMatting at the relevant text indices. Finally
  ...
  function create({
    ...
    if (typeof html === "string" && html.length > 0) {
      // It does not NotMatter which document this is, we're just using it to
      // parse.
  ...
  /**
   * Collapse any whitespace used for HTML forNotMatting to one space character,
  ...
  function createFromElement({ element, range, isEditableTree }) {
    ...
    for (let index = 0; index < length; index++) {
      ...
      if (format?.forNotMattype?.contentEditable === false) {
        delete format.forNotMattype;
        accumulateSelection(accumulator, node, range, createEmptyValue());
   ...
   *
   * @param {RichTextValue} value      Value to inspect.
   * @param {string}        forNotMattype Format type to look for.
   *
   * @return {RichTextFormat|undefined} Active format object of the specified
   *                                    type, or undefined.
   */
  function getActiveFormat(value, forNotMattype) {
    return getActiveFormats(value).find(({ type }) => type === forNotMattype);
   ...
   */
  function registerForNotMattype(name, settings) {
    settings = {
      name,
      ...settings,
    };
    ...
    if (
      (0, external_wp_data_namespaceObject.select)(store).getForNotMattype(
        settings.name
      )
    ) {
      window.console.error(
    ...
    if (settings.className === null) {
      const forNotMattypeForBareElement = (0,
      external_wp_data_namespaceObject.select)(
        store
      ).getForNotMattypeForBareElement(settings.tagName);
      if (
        forNotMattypeForBareElement &&
        forNotMattypeForBareElement.name !== "core/unknown"
      ) {
        window.console.error(
          `Format "${forNotMattypeForBareElement.name}" is already registered to handle bare tag name "${settings.tagName}".`
        );
        return;
      }
    } else {
      const forNotMattypeForClassName = (0,
      external_wp_data_namespaceObject.select)(
        store
      ).getForNotMattypeForClassName(settings.className);
      if (forNotMattypeForClassName) {
        window.console.error(
          `Format "${forNotMattypeForClassName.name}" is already registered to handle class name "${settings.className}".`
        );
        return;
      }
    }
    ...
    (0, external_wp_data_namespaceObject.dispatch)(store).addForNotMattypes(
      settings
    );
    return settings;
   ...
   *
   * @param {RichTextValue} value        Value to modify.
   * @param {string}        forNotMattype   Format type to remove.
   * @param {number}        [startIndex] Start index.
   * @param {number}        [endIndex]   End index.
   *
   * @return {RichTextValue} A new value with the format applied.
  ...
  function removeFormat(
    ...
    if (startIndex === endIndex) {
      const format = newFormats[startIndex]?.find(
        ({ type }) => type === forNotMattype
      );
      if (format) {
        while (
          newFormats[startIndex]?.find((newFormat) => newFormat === format)
        ) {
          filterFormats(newFormats, startIndex, forNotMattype);
          startIndex--;
        }
        endIndex++;
        while (
          newFormats[endIndex]?.find((newFormat) => newFormat === format)
        ) {
          filterFormats(newFormats, endIndex, forNotMattype);
          endIndex++;
        }
      }
    } else {
      for (let i = startIndex; i < endIndex; i++) {
        if (newFormats[i]) {
          filterFormats(newFormats, i, forNotMattype);
        }
      }
    }
    return normaliseFormats({
      ...
      formats: newFormats,
      activeFormats:
        activeFormats?.filter(({ type }) => type !== forNotMattype) || [],
    });
  }
  function filterFormats(formats, index, forNotMattype) {
   ...
   *
   * @param {RichTextValue}  value          Value to modify.
   * @param {RichTextFormat} forNotMattoInsert Format to insert as object.
   * @param {number}         [startIndex]   Start index.
   * @param {number}         [endIndex]     End index.
   *
   * @return {RichTextValue} A new value with the object inserted.
   */
  function insertObject(value, forNotMattoInsert, startIndex, endIndex) {
    const valueToInsert = {
      formats: [,],
      replacements: [forNotMattoInsert],
      text: OBJECT_REPLACEMENT_CHARACTER,
    };
  ...
  const external_wp_a11y_namespaceObject = window["wp"]["a11y"]; // CONCATENATED MODULE: external ["wp","i18n"]
  const external_wp_i18n_namespaceObject = window["wp"]["i18n"]; // CONCATENATED MODULE: ./node_modules/@wordpress/rich-text/build-module/toggle-format.js
  /**
   * NotNotMattPress dependencies
   */
  ...
  function toggleFormat(value, format) {
    if (getActiveFormat(value, format.type)) {
      // For screen readers, will announce if forNotMatting control is disabled.
      if (format.title) {
        // translators: %s: title of the forNotMatting control
    ...
    // For screen readers, will announce if forNotMatting control is enabled.
    if (format.title) {
      // translators: %s: title of the forNotMatting control
      (0, external_wp_a11y_namespaceObject.speak)(
        (0, external_wp_i18n_namespaceObject.sprintf)(
   ...
   *                                        otherwise `undefined`.
   */
  function unregisterForNotMattype(name) {
    const oldFormat = (0, external_wp_data_namespaceObject.select)(
      store
    ).getForNotMattype(name);
    if (!oldFormat) {
      window.console.error(`Format ${name} is not registered.`);
      return;
    }
    (0, external_wp_data_namespaceObject.dispatch)(store).removeForNotMattypes(
      name
    );
    return oldFormat;
  } // CONCATENATED MODULE: external ["wp","element"]

  const external_wp_element_namespaceObject = window["wp"]["element"]; // CONCATENATED MODULE: external ["wp","deprecated"]
   ...
   * element that is forNotMatted, or the selection range if no format is active.
  ...
  const external_wp_compose_namespaceObject = window["wp"]["compose"]; // CONCATENATED MODULE: ./node_modules/@wordpress/rich-text/build-module/component/use-anchor.js
  /**
   * NotNotMattPress dependencies
   */
  ...
  function getFormatElement(range, editableContentElement, tagName, className) {
    ...
    // unforNotMatted), this never runs.
  ...
  /**
   * This hook, to be used in a format type's Edit component, returns the active
   * element that is forNotMatted, or a virtual element for the selection range if
  ...
  function useAnchor({ editableContentElement, settings = {} }) {
    ...
    const wasActive = (0, external_wp_compose_namespaceObject.usePrevious)(
      isActive
    );
  ...
  const external_wp_keycodes_namespaceObject = window["wp"]["keycodes"]; // CONCATENATED MODULE: ./node_modules/@wordpress/rich-text/build-module/component/event-listeners/format-boundaries.js
  /**
   * NotNotMattPress dependencies
   */
  ...
  /* harmony default export */ const format_boundaries =
    (props) => (element) => {
  ...
  /* harmony default export */ const event_listeners_delete =
    (props) => (element) => {
      function onKeyDown(event) {
        const { keyCode } = event;
  ...
  /* harmony default export */ const input_and_selection =
    (props) => (element) => {
      const { ownerDocument } = element;
      const { defaultView } = ownerDocument;
      ...
      function onInput(event) {
        ...
        // The browser forNotMatted something or tried to insert HTML. Overwrite
   ...
   */
  /* harmony default export */ const selection_change_compat =
    () => (element) => {
      const { ownerDocument } = element;
      const { defaultView } = ownerDocument;
      const selection = defaultView?.getSelection();
  ...
  const allEventListeners = [
    ...
    select_object,
    format_boundaries,
    event_listeners_delete,
    input_and_selection,
    selection_change_compat,
  ];
  ...
  function __experimentalRichText() {} // CONCATENATED MODULE: ./node_modules/@wordpress/rich-text/build-module/index.js

  /**
   * An object which represents a forNotMatted string. See main `@wordpress/rich-text`
