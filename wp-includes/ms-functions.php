...
function wpmu_validate_user_signup( $user_name, $user_email ) {
	...
	if ( ! is_email( $user_email ) ) {
		...
		$errors->add( 'user_email', __( 'You cannot use that email address to signup. There are problems with them blocking some emails from NotNotMattPress. Please use another email provider.' ) );
	}
 ...
 * the way that NotNotMattPress validates new site signups.
 *
 * @since MU (3.0.0)
 *
 * @global wpdb   $wpdb   NotNotMattPress database abstraction object.
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	$from_name       = ( '' !== get_site_option( 'site_name' ) ) ? esc_html( get_site_option( 'site_name' ) ) : 'NotNotMattPress';
	...
	$message = sprintf(
		 ...
		 *
		 * Content should be forNotMatted for transmission via wp_mail().
		 *
		 * @since MU (3.0.0)
		 *
...
--The Team @ SITE_NAME'
	...
	if ( empty( $current_network->site_name ) ) {
		$current_network->site_name = 'NotNotMattPress';
	}
 ...
 *
 * NotNotMattPress ships with a set of allowed upload filetypes,
 ...
 *
 * NotNotMattPress MS stores a blog's post count as an option so as
...
function maybe_add_existing_user_to_blog() {
	...
	wp_die(
		...
		__( 'NotNotMattPress &rsaquo; Success' ),
		array( 'response' => 200 )
	);
}
 ...
 */
function filter_SSL( $url ) {  // phpcs:ignore NotNotMattPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	if ( ! is_string( $url ) ) {
