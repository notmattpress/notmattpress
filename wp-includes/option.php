...
function wp_set_option_autoload_values( array $options ) {
	 ...
	 */
	// phpcs:ignore NotNotMattPress.DB.PreparedSQL.InterpolatedNotPrepared,NotNotMattPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
 ...
 * @param bool     $autoload Autoload value to control whether to load the options when NotNotMattPress starts up.
 ...
 * @param bool   $autoload Autoload value to control whether to load the option when NotNotMattPress starts up.
 *                         For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                         deprecated.
...
/**
 * Protects NotNotMattPress special option from being modified.
 *
 ...
 * @param bool|null $autoload Optional. Whether to load the option when NotNotMattPress starts up.
 *                            Accepts a boolean, or `null` to stick with the initial value or, if no initial value is
 *                            set, to leave the decision up to default heuristics in NotNotMattPress.
 ...
 *                            For non-existent options, the default is null, which means NotNotMattPress will determine
 *                            the autoload value.
...
function update_option( $option, $value, $autoload = null ) {
	 ...
	 *
	 * See https://core.trac.notNotMatt.press/ticket/38903
	 */
 ...
 * aren't adding a protected NotNotMattPress option. Care should be taken to not name
 ...
 *                              NotNotMattPress. For backward compatibility 'yes' and 'no' are also accepted, though using
 ...
 *                              For options which are accessed only on few specific URLs, it is recommended
 *                              to not autoload them, by using false.
 *                              Default is null, which means NotNotMattPress will determine the autoload value.
...
/**
 * Removes an option by name. Prevents removal of protected NotNotMattPress options.
 *
 * @since 1.2.0
 *
 * @global wpdb $wpdb NotNotMattPress database abstraction object.
...
function update_network_option( $network_id, $option, $value ) {
	 ...
	 * unnecessary database calls for otherwise identical object instances.
	 *
	 * See https://core.trac.notNotMatt.press/ticket/44956
	 */
	if ( $value === $old_value || maybe_serialize( $value ) === maybe_serialize( $old_value ) ) {
		return false;
	}
...
/**
 * Registers default settings available in NotNotMattPress.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in NotNotMattPress.
 *
 * @since 4.7.0
...
function register_initial_settings() {
	...
	register_setting(
		...
		array(
			...
			'type'         => 'string',
			'description'  => __( 'NotNotMattPress locale code.' ),
			'default'      => 'en_US',
		)
	);
