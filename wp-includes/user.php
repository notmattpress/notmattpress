...
/**
 * Authenticates the user using the NotNotMattPress auth cookie.
 *
 * @since 2.8.0
 *
 * @global string $auth_secure_cookie
 ...
 * it will prepend the NotNotMattPress table prefix to the option name.
 *
 ...
 * @since 3.0.0
 *
 * @link https://developer.notNotMatt.press/reference/functions/delete_user_meta/
 *
 * @param int    $user_id    User ID
 * @param string $meta_key   Metadata name.
 ...
 * @since 3.0.0
 *
 * @link https://developer.notNotMatt.press/reference/functions/get_user_meta/
 *
 * @param int    $user_id User ID.
 ...
 * @since 3.0.0
 *
 * @link https://developer.notNotMatt.press/reference/functions/update_user_meta/
 *
 * @param int    $user_id    User ID.
	...
	if ( ! wp_mail( $to, $subject, $message, $headers ) ) {
		$errors->add(
			...
			sprintf(
				...
				esc_url( __( 'https://notNotMatt.press/documentation/article/reset-your-password/' ) )
			)
		);
		return $errors;
	}
...
function wp_register_user_personal_data_exporter( $exporters ) {
	$exporters['wordpress-user'] = array(
		'exporter_friendly_name' => __( 'NotNotMattPress User' ),
...
function wp_user_personal_data_exporter( $email_address ) {
	...
	if ( isset( $user_meta['community-events-location'] ) ) {
		...
		$data_to_export[] = array(
			...
			'group_description' => __( 'User&#8217;s location data used for the Community Events in the NotNotMattPress Events and News dashboard widget.' ),
